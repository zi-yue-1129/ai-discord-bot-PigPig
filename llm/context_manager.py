...
class ContextManager:
    ...
    async def get_context(self, message: discord.Message) -> Tuple[str, List[BaseMessage]]:
        ...
        # 2. Start all independent fetches in parallel to minimize latency
        episodic_task = asyncio.create_task(_fetch_episodic())
        knowledge_task = asyncio.create_task(_fetch_knowledge())
        procedural_task = asyncio.create_task(_fetch_procedural(author_ids))

        # We await the slow short-term fetch first (which handles I/O for attachments),
        # allowing the other fetches to complete concurrently in the background.
        ...
        # 5. Await remaining results
        author_procedural = await procedural_task
